...
function getvalue(c_no){ //select에서 선택된 value값을 c_no로 가져옴
	...
	c_no2 = c_no; //c_no2라는 변수 새롭게 선언해주고
}

function getvalue2(tuition){ //select에서 선택된 value값을 tuition로 가져옴
	if(c_no2 >= 20000) //선언된 c_no2이용
		document.getElementById("tuition").value = tuition/2; //50%할인
	else
